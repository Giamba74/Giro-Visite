...
def get_google_data(nome, indirizzo, comune):
    ...
    # Query rinforzata per evitare errori di posizionamento
    ...
    if 'giro_igt' in st.session_state:
        for i, p in enumerate(st.session_state.giro_igt):
            with st.container():
                ...
                # Campo Note Vocali
                st.session_state.giro_igt[i]['NOTE'] = st.text_area("Note visita (usa microfono):", value=p['NOTE'], key=f"note_{i}", height=70)
                ...
                with c3:
                    if st.button("✅ FATTO", key=f"f_{i}"):
                        st.success("Salvataggio...")

        # --- INVIO EMAIL RIEPILOGO ---
        if st.button("📧 FINISCI GIORNATA E INVIA NOTE"):
            note_presenti = [f"- {p['NOME']}: {p['NOTE']}" for p in st.session_state.giro_igt if p['NOTE'].strip()]
            if note_presenti:
